...
import com.gzstarry.shiro.jwt.JwtToken;
...
public class ShiroRealm extends AuthorizingRealm {
     ...
     * 1、登录成功后将用户的JWT生成的token存储到redis里面
     * 2、当该用户再次请求时，通过JWTFilter层层校验之后会进入到doGetAuthenticationInfo进行身份验证
     * 3、当该用户这次请求JWTToken值还在生命周期内，则会重新覆盖redis中的token值，过期时间重新计算
     * 4、当该用户这次请求jwt生成的token值已经超时，但该token对应redis中的k还是存在，则表示该用户一直在操作只是JWT的token失效了，程序会重新生成token并覆盖redis中的值，过期时间重新计算
     * 5、当该用户这次请求jwt在生成的token值已经超时，并在redis中不存在对应的k，则表示该用户账户空闲超时，返回用户信息已失效，请重新登录。
    ...
    public boolean jwtTokenRefresh(String userId) {
        // 从redis中获取token
        String redisToken = (String)redisUtil.get("lunhui:sysUser:" + userId);
        if (StringUtils.isNotBlank(redisToken)) {
            // 校验token是否过期
            if (jwtUtils.isTokenExpired(redisToken)) {
                String newToken = jwtUtils.createToken(userId);
                redisUtil.set("lunhui:sysUser:" + userId, newToken, jwtUtils.getExpire());
            } else {
                redisUtil.set("lunhui:sysUser:" + userId, redisToken, jwtUtils.getExpire());
            }
            return true;
        }
        return false;
